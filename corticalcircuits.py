...
def estimate_fano(t, spikes, window_time=0.1):
    ...
    window_pos_list = np.arange(0, t.size - n_window_steps)
    counts = np.zeros(window_pos_list.size)

    for pos in window_pos_list:
        counts[pos] = np.sum(spikes[pos:pos+n_window_steps] > 0)
...
def q3e():
    ...
    print(np.sum(output_spikes > 0) / t_end)
